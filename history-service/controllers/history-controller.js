...
export async function createHistory(req, res) {
  try {
    ...
    if (quesId && userId1 && userId2) {
      ...
      return res
        .status(400)
        .json({ message: "quesId or userId1 or userId2 may be missing!" });
    }
  } catch (err) {
...
export async function getHistoryById(req, res) {
  try {
    ...
    return res
      .status(500)
      .json({ message: "Database failure when getting hist!" });
  }
}
...
export async function getHistoryByUserId(req, res) {
  try {
    ...
    if (resp.err) {
      return res
        .status(400)
        .json({ message: "Could not get history by userId" });
    } else if (resp) {
      return res.status(200).json({
    ...
    return res
      .status(500)
      .json({ message: "Database failure when getting hist by userId!" });
  }
}
...
export async function updateHistory(req, res) {
  try {
    ...
    if (id && data.answer) {
      ...
      return res
        .status(400)
        .json({ message: "History ID or answer is missing!" });
    }
  } catch (err) {
    return res
      .status(500)
